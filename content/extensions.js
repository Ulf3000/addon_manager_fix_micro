...
XPCOMUtils.defineLazyModuleGetter(this, "Preferences", "resource://gre/modules/Preferences.jsm");
XPCOMUtils.defineLazyModuleGetter(this, "Experiments", "resource:///modules/experiments/Experiments.jsm");
XPCOMUtils.defineLazyPreferenceGetter(this, "WEBEXT_PERMISSION_PROMPTS", "extensions.webextPermissionPrompts", false);
XPCOMUtils.defineLazyPreferenceGetter(this, "ALLOW_NON_MPC", "extensions.allow-non-mpc-extensions", true);
XPCOMUtils.defineLazyPreferenceGetter(this, "SUPPORT_URL", "app.support.baseURL", "", null, val => Services.urlFormatter.formatURL(val));
...
var gViewController = {
	...
	parseViewId(aViewId) {
		...
		return {
			type: viewType,
			param: decodeURIComponent(viewParam)
		};
	},
...
var gLegacyView = {
	...
	async show(type, request) {
		...
		sortElements(elements, sortState, sortAsc);
		for (let element of elements) {
...
var amfSort = {
	...
	init: function () {
		...
		butA.sortby = ["name"];
		butA.addEventListener("command", amfSort.changeSort, false);
		amfSort.butA = butA;
		...
		butB.addEventListener("command", amfSort.changeSort, false);
		amfSort.butB = butB;
	...
	getSortButtons: function () {
		return document.getElementById('amf-sort').getElementsByTagName('button');
	},
	changeSort: function (e) {
		var button = e.target;
		if (button === amfSort.butA) {
			amfSort.butA.setAttribute('checkState', button.getAttribute('checkState') == ("1" || null) ? "2" : "1");
			amfSort.butA.setAttribute('checked', "true");

			amfSort.butB.removeAttribute('checked');
			amfSort.butB.removeAttribute('checkState');
		} else {
			amfSort.butB.setAttribute('checkState', button.getAttribute('checkState') == ("2" || null) ? "1" : "2");
			amfSort.butB.setAttribute('checked', "true");

			amfSort.butA.removeAttribute('checked');
			amfSort.butA.removeAttribute('checkState');
		}

		sortState.length = 0;
		sortState.push(button.sortby[0]);
		if (sortUiState == true)
			sortState.push("uiState");
		sortState.reverse();
		sortAsc = button.getAttribute('checkState') == "1";

		amfSort.applySort();
	},
	applySort: function () {
		var list = document.getElementById('addon-list');
		var items = Array.slice(list.childNodes, 0); //convert element-list to array
		sortElements(items, sortState, sortAsc);
